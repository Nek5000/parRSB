...
static int re2_coord(double **coord_, uint nelt, int nv, MPI_File file,
                     struct comm *c) {
  uint rank = c->id;
  uint size = c->np;
  MPI_Comm comm = c->c;
...
static int re2_boundary(uint *nbcs_, long long **bcs_, uint nelt, int nv,
  ...
  slong nbcsl = nbcs / size;
  uint nrem = nbcs - nbcsl * size;
