...
int vec_copy(genmap_vector y, genmap_vector x) {
  /* Asserts:
       - size y = size x
  */
...
int fiedler(struct rsb_element *elems, uint lelt, int nv, int max_iter,
  ...
  do {
    ...
    if (algo == 0)
      ...
      iter =
          inverse(fiedler, &gl, d, initv, gsc, max_iter, 0 /* grammian */, buf);
